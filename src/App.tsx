  ...
  return (
    <>
      ...
      <header className="topbar">
        ...
        <div className="header-controls"><button className={`switch-control ${darkModeEnabled ? "is-on" : ""}`} type="button" role="switch" aria-checked={darkModeEnabled} title="Toggle dark mode. The first visit follows your system preference." onClick={() => setTheme(darkModeEnabled ? "light" : "dark")}><span>Dark mode</span><i aria-hidden="true" /></button><button className={`switch-control ${showStatusSymbols ? "is-on" : ""}`} type="button" role="switch" aria-checked={showStatusSymbols} title="Show or hide text symbols alongside status colours." onClick={() => setShowStatusSymbols((value) => !value)}><span>Status symbols</span><i aria-hidden="true" /></button></div>
      </header>
      ...
      <main id="main-content" tabIndex={-1}>
        ...
        <section className="keyboard-section" id="keyboard">
          <div className="section-heading"><div><p className="eyebrow">Interactive reference</p><h2>Keyboard map</h2><p>Filter the reference context, then select modifiers directly on the keyboard. Command purpose affects guidance only; it never changes whether the browser can receive the event.</p></div><div className="live-readout"><span className="pulse" aria-hidden="true" /><small>Live</small><strong>{shortcut.display}</strong><button type="button" title={isListening ? "Pause detection to use keyboard shortcuts normally on this page." : "Resume live shortcut detection."} aria-pressed={isListening} onClick={() => setIsListening((value) => !value)}>{isListening ? "Pause" : "Resume"}</button></div></div>
