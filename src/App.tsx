...
export default function App() {
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      if ((event.target as HTMLElement | null)?.closest?.(".demo-overlay")) return;
  ...
  }, [contributionActive, contributionIndex, detectedEnvironment.os, environment.browser, environment.layout, environment.os, isListening]);
    ...
    <>
      ...
      <main id="main-content" tabIndex={-1}>
        <section className="hero" id="explore">
          ...
          <div className={`listener ${isListening ? "listener-on" : "listener-off"} ${listenerFlash ? "listener-detected" : ""}`} aria-live="polite"><div className="listener-indicator"><span className="pulse" aria-hidden="true" /><div><strong>{isListening ? "Listening continuously" : "Detection paused"}</strong><small>{isListening ? "Press another shortcut anywhere on this page." : "Resume to detect keyboard shortcuts."}</small></div></div><kbd>{shortcut.display}</kbd><button type="button" aria-pressed={isListening} onClick={() => setIsListening((value) => !value)}>{isListening ? "Pause" : "Resume"}</button></div>
        ...
        <section className="contribute-section" id="contribute">
          ...
          {!contributionActive ? <div className="contribute-start"><ul><li>The current browser and operating system are detected.</li><li>Confirm the keyboard layout in Keyboard map before starting.</li><li>Close, quit, reload, and history-navigation shortcuts are excluded.</li><li>No account identifier or IP address is added to the public dataset.</li><li>Results remain unverified until independently reproduced.</li></ul><button className="primary-button" type="button" onClick={startContribution}>Start guided test</button></div> : <>
            <div className={`guided-test ${contributionFlash ? "test-success" : ""}`} aria-live="polite"><div className="test-config"><span>Test configuration</span><strong>{detectedEnvironment.browser} {detectedEnvironment.browserVersion} · {detectedEnvironment.os} · {layoutLabel(environment.layout)}</strong><small>Browser and OS detected; layout taken from Keyboard map.</small></div><div className="test-target"><small>Test {contributionIndex + 1} of {contributionShortcuts.length}</small><kbd>{displayedExpectedContribution}</kbd><p>{currentContributionResult?.autoDetected ? "Detected and marked Worked automatically." : observedContributionKey ? `Latest event was ${observedContributionKey}. Try the requested combination or choose a result manually.` : "Press the combination. A matching event will be marked Worked."}</p></div><ResultButtons value={currentContributionResult?.result ?? null} onChange={(result) => setContributionResult(contributionIndex, result)} /><button className="primary-button" type="button" disabled={!currentContributionResult || contributionFinished} onClick={nextContribution}>{contributionResults.length >= minimumContributionSize ? "Save and continue testing" : "Next shortcut"}</button></div>
            {contributionResults.length ? <div className="contribution-log"><div><h3>Tested combinations</h3><p>Correct any result at any time. Color and text always carry the same status.</p></div><ol>{contributionResults.map((result, index) => <li className={`result-${result.result}`} key={`${index}-${result.shortcut}`}><div><span>{index + 1}</span><kbd>{result.shortcut}</kbd>{result.autoDetected ? <small>Auto-detected</small> : <small>Manually set</small>}</div><ResultButtons compact value={result.result} onChange={(value) => setContributionResult(index, value)} /></li>)}</ol>{contributionReady ? <div className="contribution-ready"><strong>{contributionFinished ? "All available combinations tested." : "Minimum reached — you can contribute now or keep testing."}</strong><a className="primary-button" href="https://github.com/reboot81/canibind/issues/new">Review contribution ↗</a></div> : null}</div> : null}
          </>}
        </section>
