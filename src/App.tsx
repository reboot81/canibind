  ...
  return (
    <>
      ...
      <header className="topbar">
        ...
        <div className="header-controls"><div className="theme-options" role="group" aria-label="Theme"><span>Theme</span><button type="button" aria-pressed={theme === "system"} title="Follow the system theme" onClick={() => setTheme("system")}>Auto</button><button type="button" aria-pressed={theme === "light"} title="Use light theme" onClick={() => setTheme("light")}>Light</button><button type="button" aria-pressed={theme === "dark"} title="Use dark theme" onClick={() => setTheme("dark")}>Dark</button></div><button className="symbol-button" type="button" aria-pressed={showStatusSymbols} title="Show or hide text symbols alongside status colours." onClick={() => setShowStatusSymbols((value) => !value)}>Status symbols: {showStatusSymbols ? "on" : "off"}</button></div>
      </header>
      ...
      <main id="main-content" tabIndex={-1}>
        ...
        <section className="keyboard-section" id="keyboard">
          ...
          <div className="map-guidance" aria-live="polite"><article className={`map-guidance-${reference?.capability ?? "lack-of-data"}`}><span>Can I bind it?</span><strong>{statusLabel(reference?.capability ?? "lack-of-data")}</strong><p>{reference?.note ?? "No verified observation exists yet for this exact browser, operating system, version, and keyboard layout."}</p><small>Evidence: {reference?.evidence ?? "none"}</small></article><article className={`map-guidance-${recommendation.value}`}><span>Should I bind it?</span><strong>{statusLabel(recommendation.value)}</strong><p>{recommendation.reason}</p><small>Purpose: {intents.find((item) => item.value === intent)?.label ?? intent}</small></article></div>
        </section>
