...
export default function Demo({ open, onClose, platform }: DemoProps) {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      const target = event.target as HTMLElement | null; const key = event.key.toLowerCase(); const primaryModifier = event.ctrlKey || event.metaKey;
      ...
      if (primaryModifier && key === "p") { event.preventDefault(); runCommand("print"); return; }
      if (primaryModifier && key === "k") { event.preventDefault(); setCommandOpen((current) => !current); setCommandQuery(""); setCommandIndex(0); setAnnouncement("Command palette toggled"); return; }
      if (primaryModifier && key === "f") { event.preventDefault(); searchRef.current?.focus(); setAnnouncement("Ticket search focused"); return; }
      if (event.altKey && !event.shiftKey && key === "n") { event.preventDefault(); runCommand("new"); return; }
      if (event.altKey && !event.shiftKey && key === "t") { event.preventDefault(); runCommand("tickets"); return; }
      if (event.altKey && event.shiftKey && sortShortcut[key]) { event.preventDefault(); changeSort(sortShortcut[key]); return; }
      if (functionNavigation[event.key]) { event.preventDefault(); activateView(functionNavigation[event.key]); return; }
